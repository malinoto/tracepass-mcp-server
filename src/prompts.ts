    ...
    ({ passportId }) =>
      userPrompt(
        `Audit the TracePass Digital Product Passport with id "${passportId}".\n\n` +
          ...
          `4. Note where a filled value's backing is weak. A field may carry \`sourceProvider\` naming the database it came from — \`eprel\` (the EU energy-label registry), \`open-food-facts\`, \`pubchem\`, \`vies\`, \`echa\`, \`climatiq\`, \`gs1\` — with \`sourceUrl\` pointing at that record. Those are the strongest values: an official register vouches for them. A value with \`sourceProvider: "research"\` was assembled from web search and deserves a closer look. A value with no \`sourceProvider\` at all was either entered by a person (\`source: manual\`, \`supplier\`, \`company\`) or produced by the extraction agent from an uploaded document — and note that provenance is also dropped when the pipeline altered a provider's value, so an absent \`sourceProvider\` never means "no provider was consulted", only that none vouches for the stored value. Do not treat a missing \`sourceProvider\` as a defect; treat a low-confidence value with no provider as the one worth verifying first.\n` +
          `5. State plainly whether the passport is ready to publish, and if not, the exact gaps to close.\n` +
    ...
    ({ passportId }) =>
      userPrompt(
        `Do a compliance gap-check on the TracePass passport "${passportId}".\n\n` +
          ...
          `4. If a finding targets a field that IS filled (a format issue, or a value the reviewer should re-check), fetch tracepass_passports (action: get, format: full) and say where that value came from: \`sourceProvider\` names the database behind it (\`eprel\`, \`open-food-facts\`, \`pubchem\`, \`vies\`, \`echa\`, \`climatiq\`, \`gs1\`) with \`sourceUrl\` linking the record. A value an official register vouches for is cheap to confirm; one with \`sourceProvider: "research"\` or none at all is where a reviewer's time is best spent. An absent \`sourceProvider\` is not itself a gap — it also appears when the pipeline changed a provider's value before storing it.\n` +
          `5. End with the one-line verdict and the single most important thing to fix first. If \`conditionalCoverage\` is "static-only", say so — it means no binding conditional rules exist for this category yet, not that the passport is fully future-proof.\n` +
